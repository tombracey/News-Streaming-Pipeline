...
def test_get_queue_url_from_env(mock_getenv):
    mock_getenv.return_value = "https://sqs.eu-west-2.amazonaws.com/12345/test_queue"    
...
def test_get_queue_url_from_aws(mock_getenv, mock_boto3_client):
    ...
    mock_sqs = MagicMock()
    mock_sqs.get_queue_url.return_value = {"QueueUrl": "https://sqs.eu-west-2.amazonaws.com/12345/test_queue"}
...
def test_lambda_handler(mock_get_articles, mock_send_to_sqs):
    mock_get_articles.return_value = [
        {"webTitle": "Lambda Test", "webUrl": "https://example.com", "webPublicationDate": "2025-01-01T00:00:00Z"}
    ]
